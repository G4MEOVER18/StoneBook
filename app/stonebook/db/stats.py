...
class Statistik:
    ...
    by_erstellt_am_tertial: dict[str, int] = field(default_factory=dict)
    ...
    def as_dict(self) -> dict:
        return {
            ...
            "by_erstellt_am_tertial": dict(self.by_erstellt_am_tertial),
...
def _count_erstellt_am_tertial(conn: sqlite3.Connection) -> dict[str, int]:
    """Zaehlt Objekte pro ``erstellt_am``-Tertial (T1..T3), ueber alle Jahre aggregiert.

    Spiegelt :func:`_count_funddatum_tertial` um die Erfassungs-Achse und sitzt
    granular zwischen :func:`_count_erstellt_am_quartal` (3-Monats-Bloecke) und
    :func:`_count_erstellt_am_halbjahr` (6-Monats-Bloecke): teilt das Kalenderjahr
    in drei gleiche 4-Monats-Bloecke T1=Jan..Apr, T2=Mai..Aug, T3=Sep..Dez.
    Zeigt die typische Digitalisierungs-Kampagnen-Dreiteilung auf der Erfassungs-
    Achse: T1-Winter-/Fruehjahrs-Indoor-Phase mit Tucson-Nachbearbeitung,
    T2-Uebergangs-/Sommer-Phase mit reduzierter Erfassung waehrend der Feld-
    Saison, T3-Herbst-/Winter-Auslauf mit Muenchen-Nachbereitung.
    ...
    ``((monat - 1) / 4) + 1`` abgeleitet, identisch zur Funddatum-Tertial-
    Formel.
    ...
    / :func:`_count_erstellt_am_halbjahr`, damit reine Jahres-Formen ohne
    Monatsteil und pathologische Monats-Codes 00/13..99 automatisch herausfallen.
    """
    sql = (
        "SELECT ((CAST(substr(erstellt_am, 6, 2) AS INTEGER) - 1) / 4 + 1) AS t, "
        "       COUNT(*) AS n FROM objects "
        "WHERE erstellt_am IS NOT NULL AND TRIM(erstellt_am) != '' "
        "AND substr(erstellt_am, 1, 4) GLOB '[0-9][0-9][0-9][0-9]' "
        "AND substr(erstellt_am, 6, 2) GLOB '[0-1][0-9]' "
...
def _count_geaendert_am_quartal(conn: sqlite3.Connection) -> dict[str, int]:
    """Zaehlt Objekte pro ``geaendert_am``-Quartal (Q1..Q4).

    Spiegelt :func:`_count_funddatum_quartal` und :func:`_count_erstellt_am_quartal`
    um die Aenderungs-Achse und schliesst damit die Quartals-Trias auf allen
    drei Zeit-Achsen (Funddatum / erstellt_am / geaendert_am) ab. Zeigt die
    typische Pflege-Sitzungen-Rhythmik in Quartals-Schueben parallel zur
    Erfassungs-Achse (Q1-Boersen-Nachbereitung Tucson, Q4-Winter-Pflege
    Muenchen/Sainte-Marie-aux-Mines), aber als zweite unabhaengige Achse -
    ein Q3-erfasstes Stueck kann Q1-nachbearbeitet worden sein (KI-Analyse-
    Uebernahme nach Boersen-Termin, XRD-Nachtrag nach Labor-Sitzung).
    Komplementaer zum geaendert_am_quartal_in-Filter (Listen-Drill-down):
    hier die Verteilungs-Sicht ueber die vier Kalender-Quartale.
    ...
    Monatsteil. Substring-Praefix-Guards und ``BETWEEN 1 AND 12``-Guard sind
    identisch zu :func:`_count_geaendert_am_monat` und zum
    geaendert_am_quartal_in-Filter, damit reine Jahres-Formen ohne Monatsteil
    und pathologische Monats-Codes 00/13..99 automatisch herausfallen.
    """
    sql = (
        "SELECT ((CAST(substr(geaendert_am, 6, 2) AS INTEGER) - 1) / 3 + 1) AS q, "
        "       COUNT(*) AS n FROM objects "
        "WHERE geaendert_am IS NOT NULL AND TRIM(geaendert_am) != '' "
        "AND substr(geaendert_am, 1, 4) GLOB '[0-9][0-9][0-9][0-9]' "
        "AND substr(geaendert_am, 6, 2) GLOB '[0-1][0-9]' "
...
def _sum_by_geaendert_am_quartal(conn: sqlite3.Connection, value_sql: str,
                                 extra_where: str = "") -> list[tuple[str, float]]:
    """Aggregiert ``SUM(value_sql)`` gruppiert nach ``geaendert_am``-Quartal (Q1..Q4).

    Spiegelt :func:`_sum_by_erstellt_am_quartal` auf die Aenderungs-Achse
    und schliesst die Quartals-Sum-Reihe auf der dritten Zeit-Achse
    (Fund/Erfassung/Aenderung). Bei nie-aktualisierten Alt-Eintraegen
    konvergiert die Quartals-Spitze auf die Erfassungs-Quartals-Spitze
    (``geaendert_am == erstellt_am`` im ``repository._now()``-Pfad); bei
    aktiv nachgepflegten Stuecken driftet sie in das aktuelle Pflege-
    Quartal ab und beziffert damit den wertlichen/gewichtmaessigen
    Schwerpunkt der letzten Datenpflege-Kampagnen-Rhythmik. Komplementaer
    zu :func:`_count_geaendert_am_quartal` (Anzahl - Pflege-Kampagnen-
    Rhythmik) und :func:`_sum_by_geaendert_am_monat` (Pflege-Saison auf
    feinerer Monats-Achse).

    Quartal wird aus dem Monatsteil per ``((monat - 1) / 3) + 1`` abgeleitet:
    Q1=Jan..Maerz, Q2=Apr..Jun, Q3=Jul..Sep, Q4=Okt..Dez - Ganzzahl-Division
    in SQLite ist per Definition truncating, konsistent zum Python-Aequivalent
    ``(m - 1) // 3 + 1`` und zur :func:`_count_geaendert_am_quartal`-/
    ``geaendert_am_quartal_in``-Filter-Konvention.
    ...
    und :func:`_count_geaendert_am_quartal`).
    ...
    if extra_where:
        where = f"{where} AND {extra_where}"
    sql = (
        f"SELECT ((CAST(substr(geaendert_am, 6, 2) AS INTEGER) - 1) / 3 + 1) AS q, "
        f"       SUM({value_sql}) AS w "
        f"FROM objects WHERE {where} "
        f"GROUP BY q HAVING w > 0 "
        f"ORDER BY w DESC, q ASC"
...
def compute_statistics(conn: sqlite3.Connection, top_fundorte: int = 10,
    ...
    # Erfassungs-Tertial-Histogramm (T1..T3, ueber alle Jahre aggregiert):
    # spiegelt by_funddatum_tertial auf die Erfassungs-Achse und sitzt granular
    # zwischen by_erstellt_am_quartal (3-Monats-Bloecke) und by_erstellt_am_halbjahr
    # (6-Monats-Bloecke). Teilt das Kalenderjahr in drei gleiche 4-Monats-Bloecke
    # (T1=Jan..Apr, T2=Mai..Aug, T3=Sep..Dez) und zeigt die typische Digitalisierungs-
    # Kampagnen-Dreiteilung: T1=Winter-/Fruehjahrs-Indoor-Phase mit Tucson-
    # Nachbearbeitung, T2=Uebergangs-/Sommer-Phase mit reduzierter Erfassung
    # waehrend der Feld-Saison, T3=Herbst-/Winter-Auslauf mit Muenchen-
    # Nachbereitung. Reihenfolge T1->T3 chronologisch.
    st.by_erstellt_am_tertial = _count_erstellt_am_tertial(conn)
    ...
    # Aenderungs-Quartals-Rhythmik (Q1..Q4, ueber alle Jahre aggregiert):
    # spiegelt by_funddatum_quartal / by_erstellt_am_quartal auf die
    # Aenderungs-Achse und schliesst die Quartals-Trias auf allen drei Zeit-
    # Achsen ab. Zeigt die typische Pflege-Sitzungen-Rhythmik in Quartals-
    # Schueben parallel zur Erfassungs-Achse (Q1-Boersen-Nachbereitung
    ...
    st.gewicht_pro_geaendert_am_wochentag = _sum_by_geaendert_am_wochentag(
        conn, "Gewicht_g", extra_where=gewicht_where)
    # Aenderungs-Quartals-Ergiebigkeit: spiegelt wert_/gewicht_pro_erstellt_am_
    # quartal auf die Aenderungs-Achse und schliesst die Quartals-Sum-Reihe
    # auf der dritten Zeit-Achse (Fund/Erfassung/Aenderung). Bei nie-
    # aktualisierten Alt-Eintraegen konvergiert die Pflege-Quartals-Spitze auf
    # die Erfassungs-Quartals-Spitze; bei aktiv nachgepflegten Stuecken
    # driftet sie in das aktuelle Pflege-Quartal ab und beziffert damit den
    # wertlichen/gewichtmaessigen Schwerpunkt der letzten Datenpflege-
    # Kampagnen-Rhythmik. Komplementaer zu by_geaendert_am_quartal (Anzahl)
    # und zu wert_/gewicht_pro_geaendert_am_monat (Pflege-Saison auf feinerer
