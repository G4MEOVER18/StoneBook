...
def test_parse_range_leading_currency_prefix_uah_ukrainische_hryvnia():
    """Ukrainische Hryvnia ``UAH`` als ISO-4217-Code-Regional-Waehrung fuer
    Mineralien-Sammler.

    Die Ukraine ist die weltweit fuehrende Sammler-Quelle fuer Gem-
    Qualitaets-Topaz und Morion-/Rauchquarz aus dem Volyn-Pegmatit-Feld
    (Wolhynien, Zhytomyr-Oblast; Volodarsk-Volynskyi/Kupriwka-Grube mit
    champagner- bis blaugrauem Topaz-Einzelkristallen bis Meter-Groesse,
    weltweit einziges Vorkommen der Groessen-Klasse), fuer Beryll-
    Aquamarin/Heliodor aus Wolhynien, fuer die Kryvyi Rih-Eisenerz-Region
    und fuer die Nikopol-Manganerz-Region (Rhodochrosit-/Pyrolusit-
    Assoziationen). Das ``₴``-Symbol ist bereits in der Symbol-Menge,
    aber die ISO-4217-Code-Form fehlte - Auktions-Kataloge des Kiewer
    Nationalen Naturmuseums, Direkt-Verkaeufe der ukrainischen Sammler-
    Vereine (Ukrainische Mineralogische Gesellschaft) und Boersen-Belege
    aus Sainte-Marie-aux-Mines/Muenchener Mineralientage mit ukrainischen
    Ausstellern uebernehmen die UAH-Preisstellung im Kaufbeleg.

    Bisher fielen alle Formen mit ``UAH``-Praefix UND Uncertainty-Struktur
    still auf die Fallback-Zahl-Extraktion durch (identischer Bug-Effekt
    wie bei MAD/RON/PLN/CZK/HUF/RUB/BRL/MXN/TRY/THB vor deren Aufnahme in
    die Vokabel-Liste): ``UAH 5000 ± 500`` -> ``(5000, 5000)`` (Toleranz
    verloren); ``UAH 100(2)`` -> ``(100, 2)`` (semantisch falscher Range).
    """
    # ISO-4217-Code + ±-Langform-Uncertainty. Der Praefix wird gestrippt,
    # die publizierte Toleranz laeuft in den _PLUS_MINUS_UNCERTAINTY-Zweig.
    assert csv_loaders.parse_range("UAH 5000 ± 500") == pytest.approx((4500.0, 5500.0))
    assert csv_loaders.parse_range("UAH 500 ± 50") == pytest.approx((450.0, 550.0))
    # IUCr-Kompakt-Uncertainty ``N(M)`` mit Leading-Waehrungs-Marker
    # (die publizierte Standard-Unsicherheit N(M) wird symmetrisch zur
    # ±-Form aufgeloest).
    assert csv_loaders.parse_range("UAH 5.5(3)") == pytest.approx((5.2, 5.8))
    assert csv_loaders.parse_range("UAH 100(2)") == pytest.approx((98.0, 102.0))
    # Kombination Approx-Praefix + Waehrungs-Praefix + Uncertainty in
    # beiden Reihenfolgen (Rekursion loest die Verkettung transparent auf).
    assert csv_loaders.parse_range("ca. UAH 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("UAH ca. 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("~UAH 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("circa UAH 5.5(3)") == pytest.approx((5.2, 5.8))
    # Kombination Leading-Waehrung + Uncertainty + Trailing-Approx-Marker
    # (via _APPROX_VALUE_SUFFIX-Strip in der Rekursion).
    assert csv_loaders.parse_range("UAH 500 ± 50, ca.") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("UAH 500 ± 50 geschaetzt") == pytest.approx((450.0, 550.0))
    # Case-Insensitivitaet: Excel-Autocorrect ``Uah`` und lowercase-
    # Notation ``uah`` aus Konsolen-Tools ohne Caps-Lock.
    assert csv_loaders.parse_range("uah 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("Uah 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("UaH 500 ± 50") == pytest.approx((450.0, 550.0))
    # DE-Komma-Dezimal-Locale (Suisse romande CSV-Excel-Konvention) mit
    # Leading-Waehrungs-Marker.
    assert csv_loaders.parse_range("UAH 5,5 ± 0,3") == pytest.approx((5.2, 5.8))
    assert csv_loaders.parse_range("UAH 2,65(5)") == pytest.approx((2.60, 2.70))
    # Praefix + Uncertainty + Trailing-Einheit.
    assert csv_loaders.parse_range("UAH 500 ± 50 pro Stueck") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("UAH 100 ± 2 per kg") == pytest.approx((98.0, 102.0))
    # Praefix + Uncertainty + Trailing-Klammer-Annotation (Fundort).
    assert csv_loaders.parse_range("UAH 5000 ± 500 (Volodarsk-Volynskyi)") == pytest.approx((4500.0, 5500.0))
    assert csv_loaders.parse_range("UAH 100(2) [Kryvyi Rih]") == pytest.approx((98.0, 102.0))
    # Regress-Anker: Waehrungs-Praefix vor reiner Zahl (ohne Uncertainty)
    # bleibt rueckwaerts-kompatibel - der Praefix wird gestrippt und die
    # reine Zahl-Extraktion laeuft weiter mit identischem Ergebnis.
    assert csv_loaders.parse_range("UAH 500") == (500.0, 500.0)
    assert csv_loaders.parse_range("UAH 5000") == (5000.0, 5000.0)
    # Regress-Anker: Waehrungs-Praefix vor Range-Notation.
    assert csv_loaders.parse_range("UAH 500-1000") == (500.0, 1000.0)
    assert csv_loaders.parse_range("UAH 500 to 1000") == (500.0, 1000.0)
    # Symmetrie: Code-Form UAH und Symbol-Form ₴ liefern identische
    # Ergebnisse fuer dieselbe Wert-Zeile (bisher lieferte nur die
    # Symbol-Form die Toleranz korrekt).
    assert (
        csv_loaders.parse_range("UAH 500 ± 50")
        == csv_loaders.parse_range("₴500 ± 50")
        == pytest.approx((450.0, 550.0))
    )
    # Regress-Anker: bestehende Regional-Waehrungen bleiben unveraendert
...
def test_parse_range_leading_currency_prefix_isk_islaendische_krone():
    """``ISK`` (Islaendische Krone, ISO 4217) als Leading-Waehrungs-Praefix -
    Regional-Waehrung fuer islaendische Zeolith-/Basalt-Sammler.

    Island ist eine der weltweit ergiebigsten Sammler-Quellen fuer
    Zeolithe in Basalt-Blasen und Mandelstein-Amygdaloiden: die
    Berufjord-/Teigarhorn-Fundstellen (Ost-Island, Type-Locality fuer
    Heulandit-Ca, Sammler-Quelle fuer Stilbit/Desmin, Chabasit,
    Skolezit, Mesolit, Thomsonit, Analcim, Apophyllit auf klassischem
    Basalt-Muttergestein und in Achat-/Chalzedon-Geoden), die
    Reydarfjordur-/Faskrudsfjordur-Fjorde (Ost-Island, Sammler-Quelle
    fuer Gyrolit, Okenit, Levyn, Gmelinit und weitere seltene Zeolithe),
    die Djupivogur-/Streitishvarf-Kueste (Ost-Island, Sammler-Quelle
    fuer Achat-Knollen mit Zeolith-Fuellung), die Helgustadir-Mine
    (Reydarfjordur, historische Type-Locality fuer Doppelspat-Kalzit /
    "Island-Spat" mit optisch reinen Rhomboeder-Kristallen bis 30 cm),
    die Vatnajoekull-Gletscher-Randregion (Sammler-Quelle fuer
    subglaziale Palagonit-Tuff-Assoziationen mit Zeolith-Ueberzuegen),
    die Reykjanes-Halbinsel (aktive geothermale Systeme mit Schwefel-/
    Realgar-/Auripigment-Sublimaten) und die Namafjall-/Krafla-
    Solfataren (Nord-Island, Sammler-Quelle fuer gediegen Schwefel-
    Kristalle in gelben Krusten). Die Handels-Konvention der
    islaendischen Mineralien-Haendler auf dem Kolaportid-
    Wochenendmarkt in Reykjavik, in Direkt-Verkaeufen an das
    Islenska-natturusongusagn-Naturkundemuseum (Petrurun ihre
    Steinsammlung in Reykjavik) und in Sainte-Marie-aux-Mines-/
    Muenchen-/Tucson-Auktions-Katalogen mit islaendischen Ausstellern
    ist die ISK-Preisstellung mit EUR-Umrechnungs-Hinweis (``ISK 140000
    (~EUR 1000)`` als Standard-Notation der Reykjavik-Mineralien-
    Boersen; die ISK ist die kleinste Nordische Waehrungs-Einheit mit
    ~140 ISK/EUR-Wechselkurs, sodass typische Handstueck-Preise im
    fuenfstelligen ISK-Bereich liegen).

    Bisher fielen alle Formen mit ``ISK``-Praefix UND Uncertainty-
    Struktur still auf die Fallback-Zahl-Extraktion durch (identischer
    Bug-Effekt wie bei BRL/MXN/PLN/CZK/HUF/RUB/BGN/RON/UAH/PKR/TZS/PEN/
    COP/MMK/CLP vor deren Aufnahme in die Vokabel-Liste): ``ISK 140000
    ± 14000`` -> ``(140000, 140000)`` via inverted-range-Kollaps
    (Toleranz verloren); ``ISK 500000(20000)`` -> ``(500000, 20000)``
    (semantisch falscher Range statt (480000, 520000)). Kollisionsfrei
    zu Fremdwoertern: ``ISK`` ist keine EN-/DE-Wort-Sequenz (der
    Buchstaben-Cluster ``ISK`` existiert in keinem gaengigen Vokabular
    als Wort-Anfang - EN ``ask``/``risk`` bzw. DE ``Risiko`` teilen
    zwar den ``sk``-Cluster, aber die ``\\b``-Wortgrenze vor ``ISK``
    verlangt den Wort-Anfang, sodass Suffix-``sk``-Formen nicht matchen),
    sodass die ``\\b``-Wortgrenze hinter dem Code ausschliesslich den
    ISO-Code-Praefix matcht. Case-Insensitiv spiegelt die uebrige
    Vokabel-Liste (Excel-Autocorrect ``Isk`` mit Capitalize-First-Word
    und lowercase ``isk`` aus Konsolen-Tools ohne Caps-Lock).
    """
    # ISO-4217-Code + ±-Langform-Uncertainty. Der Praefix wird gestrippt,
    # die publizierte Toleranz laeuft in den _PLUS_MINUS_UNCERTAINTY-Zweig.
    assert csv_loaders.parse_range("ISK 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("ISK 140000 ± 14000") == pytest.approx((126000.0, 154000.0))
    # IUCr-Kompakt-Uncertainty ``N(M)`` mit Leading-Waehrungs-Marker.
    assert csv_loaders.parse_range("ISK 5.5(3)") == pytest.approx((5.2, 5.8))
    assert csv_loaders.parse_range("ISK 100(2)") == pytest.approx((98.0, 102.0))
    # Kombination Approx-Praefix + Waehrungs-Praefix + Uncertainty in
    # beiden Reihenfolgen (Rekursion loest die Verkettung transparent auf).
    assert csv_loaders.parse_range("ca. ISK 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("ISK ca. 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("~ISK 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("circa ISK 5.5(3)") == pytest.approx((5.2, 5.8))
    # Kombination Leading-Waehrung + Uncertainty + Trailing-Approx-Marker.
    assert csv_loaders.parse_range("ISK 500 ± 50, ca.") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("ISK 500 ± 50 geschaetzt") == pytest.approx((450.0, 550.0))
    # Case-Insensitivitaet: Excel-Autocorrect-Capitalize und lowercase-
    # Notation aus Konsolen-Tools ohne Caps-Lock.
    assert csv_loaders.parse_range("isk 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("Isk 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("IsK 500 ± 50") == pytest.approx((450.0, 550.0))
    # DE-Komma-Dezimal-Locale mit Leading-Waehrungs-Marker.
    assert csv_loaders.parse_range("ISK 5,5 ± 0,3") == pytest.approx((5.2, 5.8))
    assert csv_loaders.parse_range("ISK 2,65(5)") == pytest.approx((2.60, 2.70))
    # Praefix + Uncertainty + Trailing-Einheit / Trailing-Klammer-
    # Annotation (Fundort).
    assert csv_loaders.parse_range("ISK 500 ± 50 pro Stufe") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("ISK 140000 ± 14000 (Teigarhorn)") == pytest.approx((126000.0, 154000.0))
    assert csv_loaders.parse_range("ISK 100(2) [Berufjord]") == pytest.approx((98.0, 102.0))
    # Vergleichs-Marker und mindestens/hoechstens-Formen mit ISK-Praefix.
    assert csv_loaders.parse_range("ISK > 500") == (500.0, None)
    assert csv_loaders.parse_range("mindestens ISK 500") == (500.0, None)
    # Regress-Anker: Waehrungs-Praefix ohne Uncertainty bleibt rueckwaerts-
    # kompatibel (reine Zahl-Extraktion nach Strip).
    assert csv_loaders.parse_range("ISK 500") == (500.0, 500.0)
    assert csv_loaders.parse_range("ISK 500-1000") == (500.0, 1000.0)
    assert csv_loaders.parse_range("ISK 500 to 1000") == (500.0, 1000.0)
    # ISK OHNE folgende Zahl faellt still auf (None, None) - spiegelt die
    # Konvention der uebrigen Waehrungs-Praefixe.
    assert csv_loaders.parse_range("ISK") == (None, None)
    assert csv_loaders.parse_range("ISK ") == (None, None)
    # Regress-Anker: bestehende Regional-Waehrungen bleiben unveraendert
    # (der neue Code ergaenzt die Vokabel-Liste, ersetzt sie nicht).
    assert csv_loaders.parse_range("CHF 500 ± 50") == pytest.approx((450.0, 550.0))
...
def test_parse_range_leading_currency_prefix_nad_namibia_dollar():
    """``NAD`` (Namibia-Dollar, ISO 4217) als Leading-Waehrungs-Praefix -
    Regional-Waehrung fuer namibische Tsumeb-Sammler.

    Tsumeb (Otavi-Bergland, Nord-Namibia) zaehlt zu den fuenf beruehmtesten
    Mineralfundstellen der Welt und ist Type-Locality fuer >70 Mineralien
    (Tsumcorit, Ludlockit, Otavit, Stottit, Mottramit, u.v.a.). Sammler-
    Referenzmaterial: Dioptas (leuchtgruene Prismen), Mimetit (klare
    orange-gelbe Kristalle), Azurit-/Malachit-Pseudomorphosen, Willemit
    (unter UV), Wulfenit, Cerussit, gediegen-Kupfer/-Silber. Weitere
    namibische Sammler-Regionen: Rossing/Namib-Wueste (Uraninit, gruener
    Turmalin, Aquamarin aus den Erongo-Bergen), Karibib/Klein Spitzkoppe
    (Rauchquarz-Zepter, Aquamarin, Schoerl-Turmalin), Uis (Kassiterit,
    Beryll), Neu Schwaben/Namib Lead Mine, Kombat Mine.
    Handels-Konvention: NAD-Preise in Direkt-Verkaeufen aus Windhoek/
    Swakopmund-Sammlergeschaeften und in geerbten Sammlungs-Etiketten
    aus der deutschen und suedafrikanischen Kolonial-Provenienz. Der NAD
    ist per Currency-Board 1:1 an den ZAR gebunden, wird in Etiketten
    aber als eigenstaendiger Code gefuehrt.

    Bisher fielen alle Formen mit ``NAD``-Praefix UND Uncertainty-
    Struktur still auf die Fallback-Zahl-Extraktion durch (identischer
    Bug-Effekt wie bei allen anderen Regional-Waehrungen vor deren
    Aufnahme in die Vokabel-Liste): ``NAD 500 ± 50`` -> ``(500, 500)``
    via inverted-range-Kollaps (Toleranz verloren); ``NAD 500(20)`` ->
    ``(500, 20)`` (semantisch falscher Range statt (480, 520)).
    Kollisionsfrei zu Fremdwoertern: ``NAD`` ist keine gaengige EN-/DE-
    Wort-Sequenz; die ``\\b``-Wortgrenze hinter dem Code matcht
    ausschliesslich den ISO-Code-Praefix. Case-Insensitiv spiegelt die
    uebrige Vokabel-Liste (Excel-Autocorrect ``Nad`` mit Capitalize-
    First-Word und lowercase ``nad`` aus Konsolen-Tools ohne Caps-Lock).
    """
    # ISO-4217-Code + ±-Langform-Uncertainty. Der Praefix wird gestrippt,
    # die publizierte Toleranz laeuft in den _PLUS_MINUS_UNCERTAINTY-Zweig.
    assert csv_loaders.parse_range("NAD 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("NAD 12500 ± 1250") == pytest.approx((11250.0, 13750.0))
    # IUCr-Kompakt-Uncertainty ``N(M)`` mit Leading-Waehrungs-Marker.
    assert csv_loaders.parse_range("NAD 5.5(3)") == pytest.approx((5.2, 5.8))
    assert csv_loaders.parse_range("NAD 100(2)") == pytest.approx((98.0, 102.0))
    # Kombination Approx-Praefix + Waehrungs-Praefix + Uncertainty in
    # beiden Reihenfolgen (Rekursion loest die Verkettung transparent auf).
    assert csv_loaders.parse_range("ca. NAD 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("NAD ca. 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("~NAD 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("circa NAD 5.5(3)") == pytest.approx((5.2, 5.8))
    # Kombination Leading-Waehrung + Uncertainty + Trailing-Approx-Marker.
    assert csv_loaders.parse_range("NAD 500 ± 50, ca.") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("NAD 500 ± 50 geschaetzt") == pytest.approx((450.0, 550.0))
    # Case-Insensitivitaet: Excel-Autocorrect-Capitalize und lowercase-
    # Notation aus Konsolen-Tools ohne Caps-Lock.
    assert csv_loaders.parse_range("nad 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("Nad 500 ± 50") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("NaD 500 ± 50") == pytest.approx((450.0, 550.0))
    # DE-Komma-Dezimal-Locale mit Leading-Waehrungs-Marker.
    assert csv_loaders.parse_range("NAD 5,5 ± 0,3") == pytest.approx((5.2, 5.8))
    assert csv_loaders.parse_range("NAD 2,65(5)") == pytest.approx((2.60, 2.70))
    # Praefix + Uncertainty + Trailing-Einheit / Trailing-Klammer-
    # Annotation (Fundort).
    assert csv_loaders.parse_range("NAD 500 ± 50 pro Stufe") == pytest.approx((450.0, 550.0))
    assert csv_loaders.parse_range("NAD 12500 ± 1250 (Tsumeb)") == pytest.approx((11250.0, 13750.0))
    assert csv_loaders.parse_range("NAD 100(2) [Erongo]") == pytest.approx((98.0, 102.0))
    # Vergleichs-Marker und mindestens/hoechstens-Formen mit NAD-Praefix.
    assert csv_loaders.parse_range("NAD > 500") == (500.0, None)
    assert csv_loaders.parse_range("mindestens NAD 500") == (500.0, None)
    # Regress-Anker: Waehrungs-Praefix ohne Uncertainty bleibt rueckwaerts-
    # kompatibel (reine Zahl-Extraktion nach Strip).
    assert csv_loaders.parse_range("NAD 500") == (500.0, 500.0)
    assert csv_loaders.parse_range("NAD 500-1000") == (500.0, 1000.0)
    assert csv_loaders.parse_range("NAD 500 to 1000") == (500.0, 1000.0)
    # NAD OHNE folgende Zahl faellt still auf (None, None) - spiegelt die
    # Konvention der uebrigen Waehrungs-Praefixe.
    assert csv_loaders.parse_range("NAD") == (None, None)
    assert csv_loaders.parse_range("NAD ") == (None, None)
    # Regress-Anker: bestehende Regional-Waehrungen bleiben unveraendert
    # (der neue Code ergaenzt die Vokabel-Liste, ersetzt sie nicht).
    assert csv_loaders.parse_range("CHF 500 ± 50") == pytest.approx((450.0, 550.0))
