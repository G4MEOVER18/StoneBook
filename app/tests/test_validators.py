...
def test_parse_iso_date_spanische_monatsnamen():
    """Spanische Monatsnamen (Sammler-Region Andalusien mit Almeria/Sierra
    Almagrera/Riotinto sowie geerbte Sammler-Notizen aus spanischsprachigen
    Katalogen und suedamerikanischen Fundstellen). ES-Monatsnamen sind alle
    ASCII (keine Diakritika), daher keine Regex-Aenderung noetig."""
    # Voll ausgeschriebene Formen (alle 12 Monate, spiegelt DE/EN/IT/FR-Testblock)
    assert parse_iso_date("enero 2024") == "2024-01-01"
    assert parse_iso_date("febrero 2024") == "2024-02-01"
    # marzo bereits IT/ES-identisch (bestehender Testfall im IT-Block)
    assert parse_iso_date("marzo 2024") == "2024-03-01"
    assert parse_iso_date("abril 2024") == "2024-04-01"
    assert parse_iso_date("mayo 2024") == "2024-05-01"
    assert parse_iso_date("junio 2024") == "2024-06-01"
    assert parse_iso_date("julio 2024") == "2024-07-01"
    # agosto bereits IT/ES-identisch (bestehender Testfall im IT-Block)
    assert parse_iso_date("agosto 2024") == "2024-08-01"
    assert parse_iso_date("septiembre 2024") == "2024-09-01"
    assert parse_iso_date("octubre 2024") == "2024-10-01"
    assert parse_iso_date("noviembre 2024") == "2024-11-01"
    assert parse_iso_date("diciembre 2024") == "2024-12-01"
    # Latin-America-Variante "setiembre" (ohne p) - RAE-akzeptiert seit 1985,
    # in Argentinien/Peru/Kolumbien die gaengigere Schreibweise
    assert parse_iso_date("setiembre 2024") == "2024-09-01"
    assert parse_iso_date("13 setiembre 2024") == "2024-09-13"
    # Tag + Monat + Jahr (typische ES-Etiketten-Notation aus Almeria/Riotinto)
    assert parse_iso_date("13 enero 2024") == "2024-01-13"
    assert parse_iso_date("13. enero 2024") == "2024-01-13"
    assert parse_iso_date("28 febrero 2024") == "2024-02-28"
    assert parse_iso_date("3 abril 2020") == "2020-04-03"
    assert parse_iso_date("13 julio 2024") == "2024-07-13"
    assert parse_iso_date("31 diciembre 1999") == "1999-12-31"
    # Separator-Varianten (Punkt/Slash/Bindestrich, spiegelt DE/EN/IT/FR)
    assert parse_iso_date("13.julio.2024") == "2024-07-13"
    assert parse_iso_date("13/julio/2024") == "2024-07-13"
    assert parse_iso_date("13-julio-2024") == "2024-07-13"
    # ES-Kurzformen (ene/abr) - ES-spezifisch, mag/giu/lug/ago/sett/ott/dic
    # bleiben IT-spezifisch, jul/oct/nov/dec ueberschneiden sich mit EN.
    assert parse_iso_date("ene 2024") == "2024-01-01"
    assert parse_iso_date("abr 2024") == "2024-04-01"
    # Kurzform + Tag + Jahr
    assert parse_iso_date("13 ene 2024") == "2024-01-13"
    assert parse_iso_date("13 abr 1999") == "1999-04-13"
    # Year-first Notation ("2024 enero", "2024/enero")
    assert parse_iso_date("2024 enero") == "2024-01-01"
    assert parse_iso_date("2024/enero") == "2024-01-01"
    assert parse_iso_date("2024-julio") == "2024-07-01"
    # Case-Insensitivitaet (Caps-Lock-Notizen aus geerbten Sammler-Etiketten)
    assert parse_iso_date("ENERO 2024") == "2024-01-01"
    assert parse_iso_date("Julio 2024") == "2024-07-01"
    assert parse_iso_date("13 JULIO 2024") == "2024-07-13"
    # Kombination mit Approx-Praefix (rekursiv via bestehende Modifikatoren)
    assert parse_iso_date("ca. julio 2024") == "2024-07-01"
    assert parse_iso_date("circa 13 julio 2024") == "2024-07-13"
    # Ungueltige Tag-Werte in ES-Notation (spiegelt DE/EN/IT/FR-Konvention)
    assert parse_iso_date("30 febrero 2024") is None
    assert parse_iso_date("31 abril 2024") is None
    # Jahr ausserhalb [1800, 2999]
    assert parse_iso_date("enero 1700") is None
    assert parse_iso_date("13 julio 3000") is None
    # Regression: bestehende DE/EN/IT/FR-Formen bleiben unveraendert
    assert parse_iso_date("13. Juni 2024") == "2024-06-13"
    assert parse_iso_date("June 13, 2024") == "2024-06-13"
    assert parse_iso_date("13 giugno 2024") == "2024-06-13"
    assert parse_iso_date("13 juin 2024") == "2024-06-13"
...
def test_parse_iso_date_spanische_saison_namen():
    ...
    Potosi Bolivien, Chuquicamata Chile, La Rinconada Peru).

    primavera=Fruehling (Monat 3, teilt sich Schreibweise/Monat mit IT),
    verano=Sommer (Monat 6, ES-spezifisch, ASCII), otono=Herbst (Monat 9,
    NFKD-Post-Strip der Standard-ES-Schreibweise "otoño" mit Tilde-N),
    invierno=Winter (Monat 12, ES-spezifisch, ASCII). Symmetrie-
    Vervollstaendigung zum bereits gepflegten ES-Monat-Block (enero..
    diciembre) in :data:`_MONTH_NAMES` (652ac1a).
    """
    # Voll-Formen Season + Jahr (spiegelt DE/EN/FR/IT-Testblock)
    assert parse_iso_date("primavera 2024") == "2024-03-01"
    ...
    assert parse_iso_date("invierno 2024") == "2024-12-01"
    # Case-Insensitivitaet (Caps-Lock-Etiketten, Titel-Case)
    assert parse_iso_date("Primavera 2024") == "2024-03-01"
    assert parse_iso_date("VERANO 2024") == "2024-06-01"
    assert parse_iso_date("Otoño 2024") == "2024-09-01"
    ...
    assert parse_iso_date("INVIERNO 2024") == "2024-12-01"
    # Year-first Notation (Ordner-Struktur, Excel-Auto-Fill)
    assert parse_iso_date("2024 verano") == "2024-06-01"
    assert parse_iso_date("2024/otoño") == "2024-09-01"
    assert parse_iso_date("2024-invierno") == "2024-12-01"
    # Praepositions-Alternante von/of (DE/EN-Prosa spiegelt auf ES-Season)
    assert parse_iso_date("verano von 2024") == "2024-06-01"
    assert parse_iso_date("invierno of 2019") == "2019-12-01"
    # Kombination mit Approx-Praefix
    assert parse_iso_date("ca. verano 2024") == "2024-06-01"
    assert parse_iso_date("circa otono 2019") == "2019-09-01"
    # Ungueltiges Jahr (ausserhalb [1800, 2999])
    assert parse_iso_date("verano 1700") is None
    assert parse_iso_date("invierno 3000") is None
    # Regress: bestehende DE/EN/FR/IT-Saisonen bleiben unveraendert
    assert parse_iso_date("Sommer 2024") == "2024-06-01"
    assert parse_iso_date("summer 2024") == "2024-06-01"
    assert parse_iso_date("Winter 2024") == "2024-12-01"
    assert parse_iso_date("printemps 2024") == "2024-03-01"
    ...
    assert parse_iso_date("estate 2024") == "2024-06-01"
    assert parse_iso_date("inverno 2024") == "2024-12-01"
    # Regress: ES-Monat-Namen bleiben unveraendert (Kollisions-Schutz zwischen
    # ES-Monat und ES-Saison, keine Ueberschneidungen).
    assert parse_iso_date("13 junio 2024") == "2024-06-13"
    assert parse_iso_date("13 enero 2024") == "2024-01-13"
...
def test_parse_iso_date_de_bibliografie_ohne_jahr_marker():
    """DE-Bibliografie-Kurzform ``o.J.`` / ``o. J.`` (ohne Jahr) liefert None.

    Semantisches Schwester-Marker zu ``o.D.`` (ohne Datum): waehrend
    ``o.D.`` das komplette Datum als unermittelbar markiert, bezeichnet
    ``o.J.`` speziell das Jahr als unbekannt. Direktes DE-Pendant zur
    lateinischen ``s.a.``-Kurzform (sine anno). Standard-Abkuerzung der
    DE-Bibliothekars-/Katalog-Tradition (RAK/RDA nach DNB), im DE-
    Antiquariats-/Auktions-Handel (ZVAB, Antiquariat Reiss & Sohn,
    Buchhandel-Katalog-Konvention: "Ort, o.J." bei nicht datierten
    Drucken) und in DE-Museumskatalog-Zitaten (Naturhistorisches Museum,
    Senckenberg, Deutsches Bergbau-Museum: geerbte Objekte ohne Erwerbs-
    Jahr). Bisher fielen die Kurzform-Varianten als "invalid Datum"
    statt "no data" in den silent-data-loss-Report, obwohl der User
    semantisch bewusst "kein Jahr verfuegbar" markiert hatte.

    Alle Varianten liefern None (nicht "invalid Datum"), und der
    Marker-Check ist case-insensitive (parse_iso_date .lower()t den
    Input vor dem Check).
    """
    # Punkt-Form ``o.J.`` / Case-Insensitivitaet
    assert parse_iso_date("o.J.") is None
    assert parse_iso_date("o.j.") is None
    assert parse_iso_date("O.J.") is None
    assert parse_iso_date("O.j.") is None
    # Punkt-Whitespace-Form ``o. J.`` / Case-Insensitivitaet
    assert parse_iso_date("o. J.") is None
    assert parse_iso_date("o. j.") is None
    assert parse_iso_date("O. J.") is None
    assert parse_iso_date("O. j.") is None
    ...
    assert "o. j." in DATE_NO_DATA_MARKERS
    # Alle neuen Marker sind lowercase (Marker-Check erfolgt via .lower())
    for marker in ("o.j.", "o. j."):
        assert marker == marker.lower()
    # Regress-Anker: das parallele DE-Schwester-Marker ``o.D.`` (ohne
    # Datum) bleibt in der Menge (die Jahr-Kurzform ergaenzt die Datum-
    # Kurzform, ersetzt sie nicht - beide koexistieren semantisch als
    # unterschiedliche Praezisions-Grade der Unsicherheit).
    assert "o.d." in DATE_NO_DATA_MARKERS
    assert "o. d." in DATE_NO_DATA_MARKERS
    # Regress-Anker: die parallelen EN/Latein-Kurzformen bleiben in der Menge
    # (der DE-Zusatz darf keine bestehenden Bibliografie-Formen verdraengen).
    ...
    # Regress-Anker: echte Fehl-Eingaben mit einzelnem ``o`` bleiben None als
    # "invalid" (kein Trigger auf die Marker-Menge, Fall-Through zur Parse-
    # Kette). Ein bares ``o`` ohne Punkt-J-Struktur ist kein Marker.
    assert parse_iso_date("Sommer 84") is None
    assert parse_iso_date("32.13.2024") is None
    assert parse_iso_date("o") is None
    assert parse_iso_date("o.") is None
