...
def test_by_erstellt_am_quartal_aus_seed_db(tmp_path):
    """Erfassungs-Quartals-Histogramm aggregiert ueber alle Jahre zu Q1..Q4.

    Spiegelt by_funddatum_quartal auf die Erfassungs-Achse: Digitalisierungs-
    Kampagnen-Rhythmik in Quartals-Schueben (Q1-Indoor Januar-Maerz mit
    Tucson-Nachbearbeitung, Q4-Winter-Digitalisierung Oktober-Dezember mit
    Muenchen-Nachbereitung). Reihenfolge Q1->Q4 aufsteigend.
    """
    from stonebook.db.database import open_db

    c = open_db(tmp_path / "e_quartal.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am) VALUES (?, ?)",
        [
            # Q1: 3x (Tucson-Nachbearbeitung + Winter-Indoor)
            ("OBJ_0001", "2024-01-15 09:00:00"),  # Q1
            ("OBJ_0002", "2023-02-20 10:15:00"),  # Q1 (Tucson-Nachbereitung)
            ("OBJ_0003", "2024-03-31 11:30:00"),  # Q1 (Maerz-Rand)
            # Q2: 1x (Fruehjahrs-Uebergang)
            ("OBJ_0004", "2024-05-15 12:00:00"),  # Q2
            # Q3: 2x (Sommer-Nachtrag nach Feld-Saison)
            ("OBJ_0005", "2020-07-15 13:15:00"),  # Q3
            ("OBJ_0006", "2021-09-30 14:00:00"),  # Q3 (Sep-Rand)
            # Q4: 3x (Muenchen-Nachbereitung + Boersen-Dezember)
            ("OBJ_0007", "2023-10-31 15:15:00"),  # Q4
            ("OBJ_0008", "2024-11-20 16:00:00"),  # Q4
            ("OBJ_0009", "2024-12-05 17:00:00"),  # Q4 (Boerse)
            # Ausgeschlossene: leer/NULL/ungueltig/reine Jahresangabe
            ("OBJ_0010", ""),
            ("OBJ_0011", None),
            ("OBJ_0012", "kein-stempel"),
    ...
    assert list(st.by_erstellt_am_quartal.items()) == [
        ("Q1", 3), ("Q2", 1), ("Q3", 2), ("Q4", 3),
    ]
    assert st.as_dict()["by_erstellt_am_quartal"] == {
        "Q1": 3, "Q2": 1, "Q3": 2, "Q4": 3,
    }
    c.close()


def test_by_erstellt_am_quartal_ignoriert_unsinnige_monatsteile(tmp_path):
    """Monat 00/13 (aus kaputten Importen) faellt aus dem Erfassungs-Quartal-Histogramm.

    Ohne den BETWEEN 1 AND 12-Guard wuerde Monat 00 als Q0-Bucket und Monat
    13 als Q5-Bucket die Erfassungs-Quartals-Statistik verzerren.
    """
    from stonebook.db.database import open_db

    c = open_db(tmp_path / "e_quartal_bad.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am) VALUES (?, ?)",
        [
    ...
    c.commit()
    st = compute_statistics(c)
    assert st.by_erstellt_am_quartal == {"Q3": 1}
    c.close()


def test_by_erstellt_am_quartal_leer(tmp_path):
    """Ohne gueltige erstellt_am-Stempel ist die Quartals-Verteilung leer."""
    from stonebook.db.database import open_db

    c = open_db(tmp_path / "e_quartal_leer.sqlite3")
    st = compute_statistics(c)
    assert st.by_erstellt_am_quartal == {}
    c.close()
...
def test_by_geaendert_am_tertial_aus_seed_db(tmp_path):
    """Aenderungs-Tertial-Histogramm aggregiert ueber alle Jahre zu T1..T3.

    Spiegelt by_funddatum_tertial / by_erstellt_am_tertial auf die Aenderungs-
    Achse und schliesst die Tertial-Trias auf allen drei Zeit-Achsen. Sitzt
    granular zwischen Quartal (3-Monats-Bloecke) und Halbjahr (6-Monats-Bloecke)
    und teilt das Kalenderjahr in drei gleiche 4-Monats-Bloecke (T1=Jan..Apr,
    T2=Mai..Aug, T3=Sep..Dez). Reihenfolge T1->T3 chronologisch aufsteigend,
    Tertiale ohne Treffer fehlen im Dict.
    """
    from stonebook.db.database import open_db

    c = open_db(tmp_path / "g_tertial.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, geaendert_am) VALUES (?, ?)",
        [
            # T1 (Jan..Apr): 2x - Winter-/Fruehjahrs-Nachpflege, Tucson-Nachbereitung
            ("OBJ_0001", "2024-01-15 09:00:00"),  # T1 (Jan-Rand)
            ("OBJ_0002", "2023-04-30 10:15:00"),  # T1 (Apr-Rand)
            # T2 (Mai..Aug): 3x - Uebergangs-/Sommer-Nachpflege
            ("OBJ_0003", "2024-05-01 11:30:00"),  # T2 (Mai-Rand)
            ("OBJ_0004", "2021-07-10 12:00:00"),  # T2
            ("OBJ_0005", "2020-08-31 13:15:00"),  # T2 (Aug-Rand)
            # T3 (Sep..Dez): 4x - Herbst-/Winter-Nachpflege, Muenchen-Nachbereitung
            ("OBJ_0006", "2020-09-01 14:00:00"),  # T3 (Sep-Rand)
            ("OBJ_0007", "2023-10-05 15:15:00"),  # T3
            ("OBJ_0008", "2023-11-20 16:00:00"),  # T3 (Muenchen)
            ("OBJ_0009", "2024-12-25 17:00:00"),  # T3 (Boerse)
            # Ausgeschlossene: leer/NULL/ungueltig/reine Jahresangabe
            ("OBJ_0010", ""),
            ("OBJ_0011", None),
            ("OBJ_0012", "kein-stempel"),
    ...
    st = compute_statistics(c)
    # Chronologisch aufsteigend nach Tertial, Tertiale ohne Treffer fehlen
    assert list(st.by_geaendert_am_tertial.items()) == [
        ("T1", 2), ("T2", 3), ("T3", 4),
    ]
    assert st.as_dict()["by_geaendert_am_tertial"] == {
        "T1": 2, "T2": 3, "T3": 4,
    }
    c.close()


def test_by_geaendert_am_tertial_ignoriert_unsinnige_monatsteile(tmp_path):
    """Monat 00/13 (aus kaputten Importen) faellt aus dem Aenderungs-Tertial-Histogramm.

    Ohne den BETWEEN 1 AND 12-Guard wuerde Monat 00 als ``((0-1)/4)+1 = 0`` in
    einen ``T0``-Bucket fallen und Monat 13 als ``((13-1)/4)+1 = 4`` in einen
    ``T4``-Bucket - beide semantisch sinnlose Verzerrungen der Aenderungs-
    Tertial-Statistik.
    """
    from stonebook.db.database import open_db

    c = open_db(tmp_path / "g_tertial_bad.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, geaendert_am) VALUES (?, ?)",
        [
            ("OBJ_0001", "2024-00-15 10:00:00"),   # Monat 0 -> ignoriert
    ...
    c.commit()
    st = compute_statistics(c)
    assert st.by_geaendert_am_tertial == {"T2": 1}
    c.close()


def test_by_geaendert_am_tertial_leer(tmp_path):
    """Ohne gueltige geaendert_am-Stempel ist die Tertial-Verteilung leer."""
    from stonebook.db.database import open_db

    c = open_db(tmp_path / "g_tertial_leer.sqlite3")
    st = compute_statistics(c)
    assert st.by_geaendert_am_tertial == {}
    c.close()


def test_by_geaendert_am_tertial_grenz_monate(tmp_path):
    """Grenz-Monate Apr/Mai und Aug/Sep landen in den erwarteten Tertialen.
    ...
    c = open_db(tmp_path / "g_tertial_grenzen.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, geaendert_am) VALUES (?, ?)",
        [
            ("OBJ_0001", "2024-04-30 09:00:00"),   # T1 (Apr-Rand)
    ...
    c.commit()
    st = compute_statistics(c)
    assert st.by_geaendert_am_tertial == {"T1": 1, "T2": 2, "T3": 1}
    c.close()
...
def test_wert_pro_funddatum_wochentag_aus_seed_db(tmp_path):
    """Wertsumme pro Funddatum-Wochentag ueber alle Wochen; absteigend nach Summe.

    Spiegelt _sum_by_funddatum_monat auf die Wochentag-Achse: waehrend die
    Monats-Sicht die Saison-Ergiebigkeit beziffert, zeigt die Wochentag-Sicht
    die Wochentag-Ergiebigkeit. Boersen-Samstag (Sa) sammelt in dem Beispiel
    das doppelte Wert-Volumen eines Feld-Sonntags (So), obwohl beide gleich
    haeufig sind - die Anzahl-Sicht (by_funddatum_wochentag) wuerde diese
    Wert-Konzentration nicht offenbaren.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpfw.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, Funddatum, Wert_CHF_roh, Wert_CHF_poliert) "
        "VALUES (?,?,?,?)",
        [
            # Samstage summieren ueber Wochen hinweg: 500 + 200 + 300 = 1000
            ("OBJ_0001", "2024-06-15", 500.0, None),   # Sa
            ("OBJ_0002", "2024-06-15", 200.0, None),   # Sa (zweiter Fund)
            ("OBJ_0003", "2024-06-22", 100.0, 200.0),  # Sa (spaeter)
            # Sonntage: 400
            ("OBJ_0004", "2024-06-16", 400.0, None),   # So
            # Donnerstag: 250
            ("OBJ_0005", "2024-06-13", 250.0, None),   # Do
            # Ohne Wert -> faellt raus
            ("OBJ_0006", "2024-06-14", None, None),    # Fr, kein Wert
            # Ohne strikte YYYY-MM-DD-Form -> ignoriert
            ("OBJ_0007", "", 999.0, None),
            ("OBJ_0008", "2024", 999.0, None),
            ("OBJ_0009", "2024-07", 999.0, None),
            ("OBJ_0010", "2024-13-01", 999.0, None),
            ("OBJ_0011", "2024-02-30", 999.0, None),   # rollover
            ("OBJ_0012", "unbekannt", 999.0, None),
        ],
    )
    c.commit()
    st = compute_statistics(c)
    # Sortierung absteigend nach Summe: Sa (1000), So (400), Do (250)
    assert st.wert_pro_funddatum_wochentag == [
        ("Sa", 1000.0),
        ("So", 400.0),
        ("Do", 250.0),
    ]
    assert st.as_dict()["wert_pro_funddatum_wochentag"] == [
        ("Sa", 1000.0), ("So", 400.0), ("Do", 250.0),
    ]
    c.close()


def test_wert_pro_funddatum_wochentag_leer(tmp_path):
    """Ohne CHF-Werte oder strikte Funddaten: leere Wochentag-Wert-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpfw_leer.sqlite3")
    st = compute_statistics(c)
    assert st.wert_pro_funddatum_wochentag == []
    c.close()


def test_wert_pro_funddatum_wochentag_tie_break_iso_dow(tmp_path):
    ...
    Wochentag-Reihenfolge (Mo->So).
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpfw_tie.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, Funddatum, Wert_CHF_roh) VALUES (?, ?, ?)",
        [
            ("OBJ_0001", "2024-06-17", 100.0),  # Mo
            ("OBJ_0002", "2024-06-15", 100.0),  # Sa
            ("OBJ_0003", "2024-06-16", 100.0),  # So
        ],
    )
    c.commit()
    st = compute_statistics(c)
...
def test_gewicht_pro_funddatum_wochentag_aus_seed_db(tmp_path):
    """Gewichtsumme pro Funddatum-Wochentag; 0/NULL ignoriert.

    Zeigt die Wert/Gewicht-Entkopplung auf der Wochentag-Achse: waehrend die
    Wert-Spitze auf einem Boersen-Samstag liegen kann, kann die Gewicht-
    Spitze auf einem Feld-Sonntag mit schweren Handstuecken liegen -
    genau der Vergleich, den das Wochentag-Paar sichtbar macht.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpfw.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, Funddatum, Gewicht_g) VALUES (?, ?, ?)",
        [
            # Sonntag: Feld-Tour, schwere Handstuecke: 800 + 400 = 1200
            ("OBJ_0001", "2024-06-16", 800.0),   # So
            ("OBJ_0002", "2024-06-23", 400.0),   # So
            # Samstag: Boersen-Tour, kleine Stuecke: 100 + 50 = 150
            ("OBJ_0003", "2024-06-15", 100.0),   # Sa
            ("OBJ_0004", "2024-06-15", 50.0),    # Sa
            # NULL/0 -> raus
            ("OBJ_0005", "2024-06-14", None),
            ("OBJ_0006", "2024-06-13", 0.0),
            # Ignoriert (nicht-strikt)
            ("OBJ_0007", "2024", 999.0),
            ("OBJ_0008", "2024-02-30", 999.0),
        ],
    )
    c.commit()
    st = compute_statistics(c)
    # Sortierung absteigend nach Summe: So (1200), Sa (150)
    assert st.gewicht_pro_funddatum_wochentag == [
        ("So", 1200.0),
        ("Sa", 150.0),
    ]
    assert st.as_dict()["gewicht_pro_funddatum_wochentag"] == [
        ("So", 1200.0), ("Sa", 150.0),
    ]
    c.close()


def test_gewicht_pro_funddatum_wochentag_leer(tmp_path):
    """Ohne Gewicht: leere Wochentag-Gewicht-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpfw_leer.sqlite3")
    st = compute_statistics(c)
    assert st.gewicht_pro_funddatum_wochentag == []
    c.close()
...
def test_wert_pro_erstellt_am_wochentag_aus_seed_db(tmp_path):
    ...
    Spiegelt wert_pro_funddatum_wochentag auf die Erfassungs-Achse: waehrend
    die Fund-Wochentag-Sicht die Tour-Ergiebigkeit zeigt, zeigt die Erfassungs-
    Wochentag-Sicht die Digitalisierungs-Ergiebigkeit. Sa/So-Marathon-Sessions
    fuer nachgereichte Katalog-Wertwellen (freizeit-nahe Sammler) heben den
    Sa-/So-Wert-Bucket ueberproportional an - die Anzahl-Sicht
    (by_erstellt_am_wochentag) wuerde diese Wert-Konzentration nicht offenbaren.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpew.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am, Wert_CHF_roh, Wert_CHF_poliert) "
        "VALUES (?,?,?,?)",
        [
            # Samstage summieren ueber Wochen hinweg: 500 + 200 + 300 = 1000
            ("OBJ_0001", "2024-06-15 10:00:00", 500.0, None),   # Sa
            ("OBJ_0002", "2024-06-15 11:00:00", 200.0, None),   # Sa (zweiter Erfassungs-Batch)
            ("OBJ_0003", "2024-06-22 09:00:00", 100.0, 200.0),  # Sa (spaeter)
            # Sonntage: 400
            ("OBJ_0004", "2024-06-16 12:00:00", 400.0, None),   # So
            # Donnerstag: 250
            ("OBJ_0005", "2024-06-13 14:00:00", 250.0, None),   # Do
            # Ohne Wert -> faellt raus
    ...
    st = compute_statistics(c)
    # Sortierung absteigend nach Summe: Sa (1000), So (400), Do (250)
    assert st.wert_pro_erstellt_am_wochentag == [
        ("Sa", 1000.0),
        ("So", 400.0),
        ("Do", 250.0),
    ]
    assert st.as_dict()["wert_pro_erstellt_am_wochentag"] == [
        ("Sa", 1000.0), ("So", 400.0), ("Do", 250.0),
    ]
    c.close()


def test_wert_pro_erstellt_am_wochentag_leer(tmp_path):
    """Ohne CHF-Werte oder strikte erstellt_am-Stempel: leere Wochentag-Wert-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpew_leer.sqlite3")
    st = compute_statistics(c)
    assert st.wert_pro_erstellt_am_wochentag == []
    c.close()


def test_wert_pro_erstellt_am_wochentag_tie_break_iso_dow(tmp_path):
    """Bei gleicher Summe sortiert der Tie-Break aufsteigend nach ISO-Wochentag.

    Mo (=1) kommt vor Sa (=6), Sa vor So (=7) - spiegelt die uebrige
    Wochentag-Reihenfolge (Mo->So), identisch zu wert_pro_funddatum_wochentag.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpew_tie.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am, Wert_CHF_roh) VALUES (?, ?, ?)",
        [
...
def test_gewicht_pro_erstellt_am_wochentag_aus_seed_db(tmp_path):
    """Gewichtsumme pro erstellt_am-Wochentag; 0/NULL ignoriert.

    Zeigt die Wert/Gewicht-Entkopplung auf der Erfassungs-Wochentag-Achse:
    waehrend die Wert-Spitze auf einem Sa-Erfassungs-Marathon liegen kann,
    kann die Gewicht-Spitze auf einem Werktags-Kleinserien-Erfassungs-Tag mit
    schweren Handstuecken liegen - genau der Vergleich, den das Wochentag-Paar
    sichtbar macht.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpew.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am, Gewicht_g) VALUES (?, ?, ?)",
        [
            # Sonntag: Marathon-Erfassung schwerer Handstuecke: 800 + 400 = 1200
            ("OBJ_0001", "2024-06-16 10:00:00", 800.0),   # So
            ("OBJ_0002", "2024-06-23 11:00:00", 400.0),   # So
            # Samstag: Kleinserien-Erfassung: 100 + 50 = 150
    ...
    st = compute_statistics(c)
    # Sortierung absteigend nach Summe: So (1200), Sa (150)
    assert st.gewicht_pro_erstellt_am_wochentag == [
        ("So", 1200.0),
        ("Sa", 150.0),
    ]
    assert st.as_dict()["gewicht_pro_erstellt_am_wochentag"] == [
        ("So", 1200.0), ("Sa", 150.0),
    ]
    c.close()


def test_gewicht_pro_erstellt_am_wochentag_leer(tmp_path):
    """Ohne Gewicht: leere Wochentag-Gewicht-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpew_leer.sqlite3")
    st = compute_statistics(c)
    assert st.gewicht_pro_erstellt_am_wochentag == []
    c.close()


def test_wert_pro_erstellt_am_quartal_aus_seed_db(tmp_path):
    """Wertsumme pro erstellt_am-Quartal (Q1..Q4); absteigend nach Summe.

    Spiegelt _sum_by_funddatum_quartal auf die Erfassungs-Achse: waehrend
    die Fund-Quartals-Sicht die Boersen-/Kampagnen-Ergiebigkeit zeigt,
    zeigt die Erfassungs-Quartals-Sicht die Digitalisierungs-Ergiebigkeit
    ueber die Kalender-Quartale. Q1-Winter-Indoor-Phase mit Nachlass-
    Digitalisierung liegt typisch bei Q1-Sammlern vorne.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpeq.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am, Wert_CHF_roh, Wert_CHF_poliert) "
        "VALUES (?,?,?,?)",
        [
            # Q1 (Jan..Maerz): Winter-Indoor + Tucson-Nachbereitung: 500+300+200 = 1000
            ("OBJ_0001", "2024-02-15 10:00:00", 500.0, None),
            ("OBJ_0002", "2023-03-20 09:00:00", 300.0, None),
            ("OBJ_0003", "2024-01-10 14:00:00", 100.0, 100.0),
            # Q3 (Jul..Sep): Sommer-Nachbereitung: 400
            ("OBJ_0004", "2020-07-15 11:00:00", 400.0, None),
            # Q4 (Okt..Dez): Muenchen-Nachbereitung: 250
    ...
    assert st.wert_pro_erstellt_am_quartal == [
        ("Q1", 1000.0),
        ("Q3", 400.0),
        ("Q4", 250.0),
    ]
    assert st.as_dict()["wert_pro_erstellt_am_quartal"] == [
        ("Q1", 1000.0), ("Q3", 400.0), ("Q4", 250.0),
    ]
    c.close()


def test_wert_pro_erstellt_am_quartal_leer(tmp_path):
    """Ohne CHF-Werte oder gueltiges erstellt_am: leere Quartals-Wert-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpeq_leer.sqlite3")
    st = compute_statistics(c)
    assert st.wert_pro_erstellt_am_quartal == []
    c.close()


def test_wert_pro_erstellt_am_quartal_tie_break_quartal_nummer(tmp_path):
    """Bei gleicher Summe sortiert der Tie-Break aufsteigend nach Quartals-Nummer."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpeq_tie.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am, Wert_CHF_roh) VALUES (?, ?, ?)",
        [
...
def test_gewicht_pro_erstellt_am_quartal_aus_seed_db(tmp_path):
    """Gewichtsumme pro erstellt_am-Quartal (Q1..Q4); 0/NULL ignoriert.

    Zeigt die Wert/Gewicht-Entkopplung auf der Erfassungs-Quartals-Achse:
    waehrend die Wert-Spitze auf einem Q1-Winter-Indoor-Quartal liegen kann,
    kann die Gewicht-Spitze auf einem Q3-Nachbereitungs-Quartal mit schweren
    Handstuecken liegen.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpeq.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am, Gewicht_g) VALUES (?, ?, ?)",
        [
            ...
            # Q1 (Jan..Maerz): kleine Winter-Indoor-Stuecke: 100 + 50 = 150
    ...
    assert st.gewicht_pro_erstellt_am_quartal == [
        ("Q3", 1200.0),
        ("Q1", 150.0),
    ]
    assert st.as_dict()["gewicht_pro_erstellt_am_quartal"] == [
        ("Q3", 1200.0), ("Q1", 150.0),
    ]
    c.close()


def test_gewicht_pro_erstellt_am_quartal_leer(tmp_path):
    """Ohne Gewicht: leere Quartals-Gewicht-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpeq_leer.sqlite3")
    st = compute_statistics(c)
    assert st.gewicht_pro_erstellt_am_quartal == []
    c.close()


def test_wert_pro_erstellt_am_tertial_aus_seed_db(tmp_path):
    """Wertsumme pro erstellt_am-Tertial (T1..T3); absteigend nach Summe.

    Spiegelt _sum_by_funddatum_tertial auf die Erfassungs-Achse und sitzt
    granular zwischen _sum_by_erstellt_am_quartal (Q1..Q4, 3-Monats-Bloecke)
    und _sum_by_erstellt_am_halbjahr (H1/H2, 6-Monats-Bloecke). Teilt das
    Kalenderjahr in drei gleiche 4-Monats-Bloecke T1=Jan..Apr, T2=Mai..Aug,
    T3=Sep..Dez und beziffert die Digitalisierungs-Ergiebigkeit ueber die
    Tertial-Dreiteilung.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpet.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am, Wert_CHF_roh, Wert_CHF_poliert) "
        "VALUES (?,?,?,?)",
        [
            # T1 (Jan..Apr): Tucson-Nachbereitung hochwertig -- 500 + 300 = 800
            ("OBJ_0001", "2024-02-15 10:00:00", 500.0, None),  # T1
            ("OBJ_0002", "2023-04-30 09:00:00", 300.0, None),  # T1 (April-Rand)
            # T2 (Mai..Aug): reduzierte Erfassung waehrend Feld-Saison
            #   200 + 60 + 100 = 360
            ...
            # T3 (Sep..Dez): Muenchen-Nachbereitung: 250 + 400 = 650
    ...
    st = compute_statistics(c)
    # Sortierung absteigend nach Summe: T1 (800), T3 (650), T2 (360)
    assert st.wert_pro_erstellt_am_tertial == [
        ("T1", 800.0),
        ("T3", 650.0),
        ("T2", 360.0),
    ]
    assert st.as_dict()["wert_pro_erstellt_am_tertial"] == [
        ("T1", 800.0), ("T3", 650.0), ("T2", 360.0),
    ]
    c.close()


def test_wert_pro_erstellt_am_tertial_leer(tmp_path):
    """Ohne CHF-Werte oder gueltiges erstellt_am: leere Tertial-Wert-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpet_leer.sqlite3")
    st = compute_statistics(c)
    assert st.wert_pro_erstellt_am_tertial == []
    c.close()


def test_wert_pro_erstellt_am_tertial_tie_break_tertial_nummer(tmp_path):
    """Bei gleicher Summe sortiert der Tie-Break aufsteigend nach Tertial-Nummer."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpet_tie.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am, Wert_CHF_roh) VALUES (?, ?, ?)",
        [
...
def test_gewicht_pro_erstellt_am_tertial_aus_seed_db(tmp_path):
    """Gewichtsumme pro erstellt_am-Tertial (T1..T3); 0/NULL ignoriert.

    Zeigt die Wert/Gewicht-Entkopplung auf der Erfassungs-Tertial-Achse:
    waehrend die Wert-Spitze auf einem T1-Tucson-Nachbereitungs-Tertial
    liegen kann, kann die Gewicht-Spitze auf einem T3-Herbst-/Winter-
    Nachbereitungs-Tertial mit schweren Feld-Handstueck-Katalogisierungen
    liegen.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpet.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, erstellt_am, Gewicht_g) VALUES (?, ?, ?)",
        [
            # T3 (Sep..Dez): schwere Nachbereitung: 800 + 400 = 1200
            ("OBJ_0001", "2024-10-15 10:00:00", 800.0),
            ("OBJ_0002", "2024-11-20 09:00:00", 400.0),
            # T1 (Jan..Apr): kleine Winter-Indoor-Stuecke: 100 + 50 = 150
            ("OBJ_0003", "2024-02-15 10:00:00", 100.0),
            ("OBJ_0004", "2024-03-01 10:00:00", 50.0),
            # NULL/0 -> raus
            ("OBJ_0005", "2024-04-15 10:00:00", None),
    ...
    assert st.gewicht_pro_erstellt_am_tertial == [
        ("T3", 1200.0),
        ("T1", 150.0),
    ]
    assert st.as_dict()["gewicht_pro_erstellt_am_tertial"] == [
        ("T3", 1200.0), ("T1", 150.0),
    ]
    c.close()


def test_gewicht_pro_erstellt_am_tertial_leer(tmp_path):
    """Ohne Gewicht: leere Tertial-Gewicht-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpet_leer.sqlite3")
    st = compute_statistics(c)
    assert st.gewicht_pro_erstellt_am_tertial == []
    c.close()
...
def test_wert_pro_geaendert_am_halbjahr_aus_seed_db(tmp_path):
    ...
    Spiegelt _sum_by_geaendert_am_quartal auf die noch groebere Zwei-Punkt-
    Aggregat-Achse: waehrend die Quartals-Sicht die Pflege-Kampagnen-
    Rhythmik der Kalender-Quartale beziffert, zeigt die Halbjahres-Sicht
    die Zaesur H1/H2 (Winter-Indoor-Pflege vs. Sommer-/Herbst-Nachpflege)
    und die Excel-Halbjahres-Buchhaltungs-/Steuer-Konvention.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpgh.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, geaendert_am, Wert_CHF_roh, Wert_CHF_poliert) "
        "VALUES (?,?,?,?)",
        [
            # H1 (Jan..Jun): Winter-Indoor-Pflege: 500+300+100+100 = 1000
            ("OBJ_0001", "2024-02-15 10:00:00", 500.0, None),
            ("OBJ_0002", "2023-03-20 09:00:00", 300.0, None),
            ("OBJ_0003", "2024-06-10 14:00:00", 100.0, 100.0),
            # H2 (Jul..Dez): Sommer-/Herbst-Pflege: 400 + 250 = 650
            ("OBJ_0004", "2020-07-15 11:00:00", 400.0, None),
            ("OBJ_0005", "2023-10-31 12:00:00", 250.0, None),
            # Ohne Wert -> raus
            ("OBJ_0006", "2024-05-15 10:00:00", None, None),
    ...
    st = compute_statistics(c)
    # Sortierung absteigend nach Summe: H1 (1000), H2 (650)
    assert st.wert_pro_geaendert_am_halbjahr == [
        ("H1", 1000.0),
        ("H2", 650.0),
    ]
    assert st.as_dict()["wert_pro_geaendert_am_halbjahr"] == [
        ("H1", 1000.0), ("H2", 650.0),
    ]
    c.close()


def test_wert_pro_geaendert_am_halbjahr_leer(tmp_path):
    """Ohne CHF-Werte oder gueltiges geaendert_am: leere Halbjahres-Wert-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpgh_leer.sqlite3")
    st = compute_statistics(c)
    assert st.wert_pro_geaendert_am_halbjahr == []
    c.close()


def test_wert_pro_geaendert_am_halbjahr_tie_break_halbjahres_nummer(tmp_path):
    """Bei gleicher Summe sortiert der Tie-Break aufsteigend nach Halbjahres-Nr."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "wpgh_tie.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, geaendert_am, Wert_CHF_roh) VALUES (?, ?, ?)",
        [
            ("OBJ_0001", "2024-02-01 10:00:00", 100.0),  # H1
...
def test_gewicht_pro_geaendert_am_halbjahr_aus_seed_db(tmp_path):
    """Gewichtsumme pro geaendert_am-Halbjahr (H1/H2); 0/NULL ignoriert.

    Zeigt die Wert/Gewicht-Entkopplung auf der Pflege-Halbjahres-Achse:
    waehrend die Wert-Spitze auf einem H1-Winter-Indoor-Pflege-Halbjahr
    liegen kann, kann die Gewicht-Spitze auf einem H2-Nachbereitung
    schwerer Feld-Handstuecke liegen.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpgh.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, geaendert_am, Gewicht_g) VALUES (?, ?, ?)",
        [
            # H2 (Jul..Dez): schwere Nachbereitung: 800 + 400 = 1200
            ("OBJ_0001", "2024-07-15 10:00:00", 800.0),
            ("OBJ_0002", "2024-10-20 09:00:00", 400.0),
            # H1 (Jan..Jun): kleine Winter-Indoor-Pflege: 100 + 50 = 150
            ("OBJ_0003", "2024-02-15 10:00:00", 100.0),
            ("OBJ_0004", "2024-06-01 10:00:00", 50.0),
            # NULL/0 -> raus
            ("OBJ_0005", "2024-04-15 10:00:00", None),
    ...
    st = compute_statistics(c)
    # Sortierung absteigend nach Summe: H2 (1200), H1 (150)
    assert st.gewicht_pro_geaendert_am_halbjahr == [
        ("H2", 1200.0),
        ("H1", 150.0),
    ]
    assert st.as_dict()["gewicht_pro_geaendert_am_halbjahr"] == [
        ("H2", 1200.0), ("H1", 150.0),
    ]
    c.close()


def test_gewicht_pro_geaendert_am_halbjahr_leer(tmp_path):
    """Ohne Gewicht: leere Halbjahres-Gewicht-Liste."""
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gpgh_leer.sqlite3")
    st = compute_statistics(c)
    assert st.gewicht_pro_geaendert_am_halbjahr == []
    c.close()
