...
def test_funddatum_quartal_in_filter(tmp_path):
    """funddatum_quartal_in waehlt diskrete Kalender-Quartale (Q1..Q4).

    Quartal wird aus dem Monatsteil per ``((monat - 1) / 3) + 1`` abgeleitet:
    Q1=1..3, Q2=4..6, Q3=7..9, Q4=10..12. Spiegelt funddatum_monat_in in der
    Aggregat-Achse fuer Sammler-Notationen wie "Q1 Tucson-Show ODER Q4
    Munich-Show" ohne dass 12 einzelne Monate aufgezaehlt werden muessen.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "quartal_in.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, Funddatum) VALUES (?, ?)",
        [
            ("OBJ_0001", "2024-01-31"),   # Q1 (Rand)
            ("OBJ_0002", "2024-02-14"),   # Q1
            ("OBJ_0003", "2024-03-31"),   # Q1 (Rand)
            ("OBJ_0004", "2023-04-01"),   # Q2 (Rand)
            ("OBJ_0005", "2023-06-15"),   # Q2 (Rand)
            ("OBJ_0006", "2022-07-01"),   # Q3
            ("OBJ_0007", "2022-09-30"),   # Q3 (Rand)
            ("OBJ_0008", "2021-10-01"),   # Q4 (Rand)
            ("OBJ_0009", "2021-12-31"),   # Q4 (Rand)
            ("OBJ_0010", "2020"),         # ohne Monatsteil
            ("OBJ_0011", None),           # NULL
            ("OBJ_0012", ""),             # leer
            ("OBJ_0013", "kein-datum"),   # kaputt
    ...
    rows = repo.list_objects(funddatum_quartal_in=[1])
    assert [r["obj_id"] for r in rows] == ["OBJ_0001", "OBJ_0002", "OBJ_0003"]
    # Q2 allein: Apr/Mai/Jun (nur April und Juni belegt)
    rows = repo.list_objects(funddatum_quartal_in=[2])
    assert [r["obj_id"] for r in rows] == ["OBJ_0004", "OBJ_0005"]
    # Q3 allein
    rows = repo.list_objects(funddatum_quartal_in=[3])
    assert [r["obj_id"] for r in rows] == ["OBJ_0006", "OBJ_0007"]
    # Q4 allein
    rows = repo.list_objects(funddatum_quartal_in=[4])
    assert [r["obj_id"] for r in rows] == ["OBJ_0008", "OBJ_0009"]
    # Mengen-Auswahl: Q1 ODER Q4 (Boersen-Kalender Tucson/Munich)
    rows = repo.list_objects(funddatum_quartal_in=[1, 4])
    assert [r["obj_id"] for r in rows] == [
        "OBJ_0001", "OBJ_0002", "OBJ_0003", "OBJ_0008", "OBJ_0009",
    ]
    # Tupel akzeptiert
    rows = repo.list_objects(funddatum_quartal_in=(2, 3))
    assert [r["obj_id"] for r in rows] == [
        "OBJ_0004", "OBJ_0005", "OBJ_0006", "OBJ_0007",
    ]
    # Alle vier Quartale entspricht "alle Objekte mit gueltigem Monatsteil"
    rows = repo.list_objects(funddatum_quartal_in=[1, 2, 3, 4])
    assert [r["obj_id"] for r in rows] == [
        "OBJ_0001", "OBJ_0002", "OBJ_0003", "OBJ_0004", "OBJ_0005",
        "OBJ_0006", "OBJ_0007", "OBJ_0008", "OBJ_0009",
    ]
    # Leere Liste -> kein Filter (alle 15 Objekte)
    rows = repo.list_objects(funddatum_quartal_in=[])
    assert len(rows) == 15
    # Kombiniert mit Jahresfilter (Schnittmenge)
    rows = repo.list_objects(funddatum_quartal_in=[1, 4], funddatum_jahr_min=2024)
    assert [r["obj_id"] for r in rows] == ["OBJ_0001", "OBJ_0002", "OBJ_0003"]
    # Kombiniert mit funddatum_monat_in (Schnittmenge Q1 ∩ {Maerz} = Maerz)
    rows = repo.list_objects(funddatum_quartal_in=[1], funddatum_monat_in=[3])
    assert [r["obj_id"] for r in rows] == ["OBJ_0003"]
    # Validierung
    import pytest as _pytest
    with _pytest.raises(ValueError, match="Unbekannte Funddatum-Quartale"):
        repo.list_objects(funddatum_quartal_in=[1, 5])
    with _pytest.raises(ValueError, match="Unbekannte Funddatum-Quartale"):
        repo.list_objects(funddatum_quartal_in=[0])
    with _pytest.raises(ValueError, match="Unbekannte Funddatum-Quartale"):
        repo.list_objects(funddatum_quartal_in=[-1])
    c.close()
...
def test_geaendert_am_quartal_in_filter(tmp_path):
    """geaendert_am_quartal_in waehlt diskrete Kalender-Quartale (Q1..Q4).

    Spiegelt erstellt_am_quartal_in / funddatum_quartal_in auf die Aenderungs-
    Achse: Pflege-Sitzungen kulminieren in Quartals-Rhythmen parallel zur
    Erfassungs-Achse (Q1-Boersen-Nachbereitung Tucson, Q4-Winter-Pflege
    Muenchen/Sainte-Marie-aux-Mines) - als zweite unabhaengige Achse: ein
    Q3-erfasstes Stueck kann Q1-nachbearbeitet worden sein (KI-Analyse-
    Uebernahme nach Boersen-Termin, XRD-Nachtrag nach Labor-Sitzung).
    Quartal wird aus dem Monatsteil per ``((monat - 1) / 3) + 1`` abgeleitet:
    Q1=1..3, Q2=4..6, Q3=7..9, Q4=10..12.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gq.sqlite3")
    c.executemany(
    ...
    repo = ObjectRepo(c)
    # Q1 allein: Jan/Feb/Maerz
    rows = repo.list_objects(geaendert_am_quartal_in=[1])
    assert [r["obj_id"] for r in rows] == ["OBJ_0001", "OBJ_0002", "OBJ_0003"]
    # Q2 allein
    rows = repo.list_objects(geaendert_am_quartal_in=[2])
    assert [r["obj_id"] for r in rows] == ["OBJ_0004", "OBJ_0005"]
    # Q3 allein
    rows = repo.list_objects(geaendert_am_quartal_in=[3])
    assert [r["obj_id"] for r in rows] == ["OBJ_0006", "OBJ_0007"]
    # Q4 allein
    rows = repo.list_objects(geaendert_am_quartal_in=[4])
    assert [r["obj_id"] for r in rows] == ["OBJ_0008", "OBJ_0009"]
    # Mengen-Auswahl: Q1 ODER Q4 (Pflege-Schuebe Boersen-Nachbereitung + Winter-Pflege)
    rows = repo.list_objects(geaendert_am_quartal_in=[1, 4])
    assert [r["obj_id"] for r in rows] == [
        "OBJ_0001", "OBJ_0002", "OBJ_0003", "OBJ_0008", "OBJ_0009",
    ]
    # Tupel akzeptiert
    rows = repo.list_objects(geaendert_am_quartal_in=(2, 3))
    assert [r["obj_id"] for r in rows] == [
        "OBJ_0004", "OBJ_0005", "OBJ_0006", "OBJ_0007",
    ]
    # Alle vier Quartale entspricht "alle Objekte mit gueltigem Monatsteil"
    rows = repo.list_objects(geaendert_am_quartal_in=[1, 2, 3, 4])
    assert [r["obj_id"] for r in rows] == [
        "OBJ_0001", "OBJ_0002", "OBJ_0003", "OBJ_0004", "OBJ_0005",
        "OBJ_0006", "OBJ_0007", "OBJ_0008", "OBJ_0009",
    ]
    # Leere Liste -> kein Filter
    rows = repo.list_objects(geaendert_am_quartal_in=[])
    assert len(rows) == 15
    # Kombiniert mit Jahresfilter (Schnittmenge Q1 ∩ 2024 = Jan/Feb/Maerz 2024)
    rows = repo.list_objects(geaendert_am_quartal_in=[1, 4], geaendert_am_jahr_min=2024)
    assert [r["obj_id"] for r in rows] == ["OBJ_0001", "OBJ_0002", "OBJ_0003"]
    # Kombiniert mit geaendert_am_monat_in (Schnittmenge Q1 ∩ {Maerz} = Maerz)
    rows = repo.list_objects(geaendert_am_quartal_in=[1], geaendert_am_monat_in=[3])
    assert [r["obj_id"] for r in rows] == ["OBJ_0003"]
    # Validierung: 0/5 sind ungueltig
    with pytest.raises(ValueError, match="Unbekannte Geaendert-am-Quartale"):
        repo.list_objects(geaendert_am_quartal_in=[1, 5])
    with pytest.raises(ValueError, match="Unbekannte Geaendert-am-Quartale"):
        repo.list_objects(geaendert_am_quartal_in=[0])
    with pytest.raises(ValueError, match="Unbekannte Geaendert-am-Quartale"):
        repo.list_objects(geaendert_am_quartal_in=[-1])
    c.close()


def test_geaendert_am_halbjahr_in_filter(tmp_path):
    """geaendert_am_halbjahr_in waehlt diskrete Kalender-Halbjahre (H1/H2).

    Spiegelt erstellt_am_halbjahr_in / funddatum_halbjahr_in in der noch
    groeberen Zwei-Punkt-Achse auf der Aenderungs-Achse: Pflege-Wellen laufen
    in Halbjahres-Bloecken parallel zur Erfassungs-Achse - aber als zweite
    unabhaengige Achse (ein H2-erfasstes Stueck kann H1-nachbearbeitet worden
    sein). Halbjahr wird aus dem Monatsteil per ``((monat - 1) / 6) + 1``
    abgeleitet: H1=Jan..Jun, H2=Jul..Dez.
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "gh.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, geaendert_am) VALUES (?, ?)",
        [
            ("OBJ_0001", "2024-01-31 09:00:00"),   # H1 (Rand)
    ...
    repo = ObjectRepo(c)
    # H1 allein: Jan..Jun
    rows = repo.list_objects(geaendert_am_halbjahr_in=[1])
    assert [r["obj_id"] for r in rows] == ["OBJ_0001", "OBJ_0002", "OBJ_0003"]
    # H2 allein: Jul..Dez
    rows = repo.list_objects(geaendert_am_halbjahr_in=[2])
    assert [r["obj_id"] for r in rows] == ["OBJ_0004", "OBJ_0005", "OBJ_0006"]
    # H1 ODER H2 = alle Objekte mit gueltigem Monatsteil
    rows = repo.list_objects(geaendert_am_halbjahr_in=[1, 2])
    assert [r["obj_id"] for r in rows] == [
        "OBJ_0001", "OBJ_0002", "OBJ_0003",
        "OBJ_0004", "OBJ_0005", "OBJ_0006",
    ]
    ...
    assert [r["obj_id"] for r in rows] == ["OBJ_0001", "OBJ_0002", "OBJ_0003"]
    # Leere Liste -> kein Filter
    rows = repo.list_objects(geaendert_am_halbjahr_in=[])
    assert len(rows) == 12
    # Kombiniert mit Jahresfilter (Schnittmenge H1 ∩ 2024 = 2024er H1)
    rows = repo.list_objects(geaendert_am_halbjahr_in=[1], geaendert_am_jahr_min=2024)
    assert [r["obj_id"] for r in rows] == ["OBJ_0001", "OBJ_0002", "OBJ_0003"]
    # Kombiniert mit geaendert_am_quartal_in (Schnittmenge H1 ∩ Q1 = Q1)
    rows = repo.list_objects(geaendert_am_halbjahr_in=[1], geaendert_am_quartal_in=[1])
    assert [r["obj_id"] for r in rows] == ["OBJ_0001", "OBJ_0002"]
    # Kombiniert mit geaendert_am_monat_in (Schnittmenge H2 ∩ {Dezember} = Dezember)
    rows = repo.list_objects(geaendert_am_halbjahr_in=[2], geaendert_am_monat_in=[12])
    assert [r["obj_id"] for r in rows] == ["OBJ_0006"]
    # Validierung: 0/3 sind ungueltig
    with pytest.raises(ValueError, match="Unbekannte Geaendert-am-Halbjahre"):
        repo.list_objects(geaendert_am_halbjahr_in=[1, 3])
    with pytest.raises(ValueError, match="Unbekannte Geaendert-am-Halbjahre"):
        repo.list_objects(geaendert_am_halbjahr_in=[0])
    with pytest.raises(ValueError, match="Unbekannte Geaendert-am-Halbjahre"):
        repo.list_objects(geaendert_am_halbjahr_in=[-1])
    c.close()
...
def test_geaendert_am_wochentag_in_filter(tmp_path):
    """geaendert_am_wochentag_in waehlt diskrete ISO-Wochentage (1=Mo..7=So).

    Spiegelt erstellt_am_wochentag_in / funddatum_wochentag_in auf die
    Aenderungs-Achse - schliesst damit die Wochentag-Filter-Trias der drei
    Zeitstempel-Spalten ab und verdrahtet das bereits vorhandene
    :func:`_count_geaendert_am_wochentag`-Aggregat aus der Statistik um den
    Listen-Drill-down ("welche Objekte habe ich Sonntags nachgepflegt?").
    Aenderungs-Wochentag ist unabhaengig vom Erfassungs-Wochentag: ein
    Werktags-erfasstes Stueck kann Sonntags in der Wochenend-Pflege redaktionell
    ueberarbeitet worden sein (Foto nachgereicht, KI-Analyse uebernommen,
    Mineral-Zuordnung nach XRD-Termin praezisiert).
    ...
    """
    from stonebook.db.database import open_db
    c = open_db(tmp_path / "g_wt_in.sqlite3")
    c.executemany(
        "INSERT INTO objects (obj_id, geaendert_am) VALUES (?, ?)",
        [
            ("OBJ_0001", "2024-07-15 09:00:00"),  # Mo (voll qualifizierter Stempel)
            ...
            ("OBJ_0004", "2024-07-18 14:30:00"),  # Do
            ("OBJ_0005", "2024-07-19"),           # Fr
            ("OBJ_0006", "2024-07-13 08:45:00"),  # Sa (Wochenend-Nachpflege)
            ("OBJ_0007", "2024-07-14 20:20:00"),  # So (Wochenend-Pflege)
            ("OBJ_0008", "2024-07"),               # ohne Tag -> faellt raus
            ("OBJ_0009", "2024"),                   # nur Jahr -> faellt raus
            ("OBJ_0010", "2024-02-30 12:00:00"),  # ungueltiger Kalender-Tag -> faellt raus
    ...
    c.commit()
    repo = ObjectRepo(c)
    # Wochenende: Samstag + Sonntag (klassische Wochenend-Pflege-Sitzung)
    rows = repo.list_objects(geaendert_am_wochentag_in=[6, 7])
    assert sorted(r["obj_id"] for r in rows) == ["OBJ_0006", "OBJ_0007"]
    # Nur Werktage (Mo..Fr)
    rows = repo.list_objects(geaendert_am_wochentag_in=[1, 2, 3, 4, 5])
    assert sorted(r["obj_id"] for r in rows) == [
        "OBJ_0001", "OBJ_0002", "OBJ_0003", "OBJ_0004", "OBJ_0005"]
    # Einzelner Wochentag: nur Sonntag
    rows = repo.list_objects(geaendert_am_wochentag_in=[7])
    assert [r["obj_id"] for r in rows] == ["OBJ_0007"]
    # Reines Datum ohne Zeitkomponente: OBJ_0003 (Mi) und OBJ_0005 (Fr)
    rows = repo.list_objects(geaendert_am_wochentag_in=[3, 5])
    assert sorted(r["obj_id"] for r in rows) == ["OBJ_0003", "OBJ_0005"]
    # Tupel akzeptiert
    rows = repo.list_objects(geaendert_am_wochentag_in=(2,))
    assert [r["obj_id"] for r in rows] == ["OBJ_0002"]
    # Leere Liste -> kein Filter
    rows = repo.list_objects(geaendert_am_wochentag_in=[])
    assert len(rows) == 13
    # Kombiniert mit Jahres-Bereich (Schnittmenge)
    rows = repo.list_objects(geaendert_am_wochentag_in=[6, 7],
                              geaendert_am_jahr_min=2024)
    assert sorted(r["obj_id"] for r in rows) == ["OBJ_0006", "OBJ_0007"]
    # Kombiniert mit Monats-Filter (Schnittmenge)
    rows = repo.list_objects(geaendert_am_wochentag_in=[7],
                              geaendert_am_monat=7)
    assert [r["obj_id"] for r in rows] == ["OBJ_0007"]
    # Validierung: 0 und 8 sind ungueltig (nicht in ISO-Range 1..7)
    import pytest as _pytest
    with _pytest.raises(ValueError, match="Unbekannte Geaendert-am-Wochentage"):
        repo.list_objects(geaendert_am_wochentag_in=[0])
    with _pytest.raises(ValueError, match="Unbekannte Geaendert-am-Wochentage"):
        repo.list_objects(geaendert_am_wochentag_in=[8])
    with _pytest.raises(ValueError, match="Unbekannte Geaendert-am-Wochentage"):
        repo.list_objects(geaendert_am_wochentag_in=[1, 9])
    c.close()
